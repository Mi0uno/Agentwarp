...
- 每次开发完成后，必须完成编译并运行可测试版本，让用户进行手工验收。
- 提交给用户验收时，必须提供运行方式、已完成的自动化验证、本地 Linux 构建结果、已触发的跨平台 GitHub Actions 构建结果（如有）和需要重点测试的范围。
...
- 本地开发与功能点验证阶段，只构建 Linux 版本；不得在本地手动触发 Windows 编译（包括安装 MSVC 工具链、跑 `cargo build --target x86_64-pc-windows-msvc` 等）。
- Windows 编译由 GitHub Actions runners 统一负责，入口为 `.github/workflows/beta_release.yml`（tag `v*-beta*` 自动触发，或在 Actions 页面手动 `workflow_dispatch`）。
- 每个功能点合并回主分支**之前**，必须至少触发一次 Windows 编译验证：push 一个临时 `v*-beta*` 标签或在 Actions 页面用 `workflow_dispatch` 手动运行 `beta_release.yml`，等待 `windows_installer`（x64 和 arm64）job 全部成功，再继续合并流程；不得仅凭 Linux 构建结果合并涉及平台相关改动的功能点。
...
- 如果改动需要平台条件分支，必须分别说明 Linux 与 Windows 的行为；本地 Linux 验证 + GitHub Actions Windows 验证共同构成跨平台验证证据。
...
- 交付汇报必须包含本地 Linux 构建结果、已触发的跨平台 GitHub Actions 构建结果（含 Windows x64 / arm64 job 链接或日志）、测试结果、缓存清理结果和 Git 提交号。
...
4. 实施改动。
5. 运行完整测试流程。
6. 完成本地 Linux 构建验证，并按需触发 GitHub Actions 跨平台（Windows x64 / arm64）构建验证。
7. 编译并运行可测试版本，交给用户手工验收。
8. 主动询问用户是否测试通过。
9. 用户确认通过后，清理编译缓存和临时产物。
10. 使用 Git 提交该功能点并合并回主分支。
11. 汇报任务分支、主分支合并结果、提交号、本地 Linux 构建结果、已触发的跨平台 GitHub Actions 构建结果、测试结果、缓存清理结果和剩余风险。
